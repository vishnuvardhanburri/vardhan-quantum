...
function listFormatter(cell) {
      ...
      <div>
        {cell.map((value, idx) => (
          <div key={value.id || idx}>
            <span style={{ color: "#2563EB", fontFamily: "monospace" }}>{value.email || value.name || String(value)}</span>
          </div>
        ))}
      </div>
    );
  ...
  return <span style={{ color: "#0A0A0A", fontWeight: 600 }}>{cell.email || cell.name || String(cell)}</span>;
}
