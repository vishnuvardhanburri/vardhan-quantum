...
use super::gates::{StateSnapshotRef, G0ContextValidity};
...
impl<V: IntegrityVerifier> G0ContextValidity for G0Validator<V> {
    fn evaluate(
        ...
        if ir.schema_version != self.current_schema_version { return AssuranceStatus::Reject; }
        if ir.tenant_scope != committed_state.tenant_id { return AssuranceStatus::Reject; }
        if config_hash != &committed_state.config_hash { return AssuranceStatus::Reject; }
        if ir.assumed_system_state_hash != committed_state.state_hash { return AssuranceStatus::Reject; }

        if current_time_ms < committed_state.generated_at_ms { return AssuranceStatus::Indeterminate; }
        
        let age = current_time_ms - committed_state.generated_at_ms;
        if age > self.max_freshness_ms { return AssuranceStatus::Timeout; }
