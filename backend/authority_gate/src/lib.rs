...
impl AuthorityGate for VardhanGate {
    fn validate_and_execute(
        ...
        info!("AuthorityGate evaluating action {} (auth: {})", action_id, evidence.authorization_id);
        
        // A7 Validation 1: Authorization tracing
        if action_id != evidence.authorization_id {
            error!("Gate failure: action_id {} does not match authorization_id {}", action_id, evidence.authorization_id);
            return Err("Authorization mismatch".to_string());
        }

        // A7 Validation 2: Assurance Result
        if evidence.final_status != ActionStatus::Pass {
            error!("Gate failure: Twin status is not Pass (was {:?})", evidence.final_status);
        ...
        if evidence.assumed_config_hash != current_config_hash {
            error!("Gate failure: Stale configuration. Evidence: {}, Current: {}", evidence.assumed_config_hash, current_config_hash);
        ...
        info!("Execution authorized. Receipt issued: {}", receipt.receipt_id);
        Ok(receipt)
    }
}
